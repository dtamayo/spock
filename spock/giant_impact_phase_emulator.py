...
from .simsetup import copy_sim, sim_subset, revert_sim_units
...
class GiantImpactPhaseEmulator():
    ...
    def __init__(self, sims, tmax=None, seed=None):
        ...
        self.tmax = self._get_orbsmax(tmax) # initialize array of max orbits for each simulation (default 1e9)
    ...
    def predict(self, reuse_inputs=True):
        while np.any([sim.t < self.tmax[i] for i, sim in enumerate(self.sims)]): # take another step if any sims are still at t < tmax
            self.step(reuse_inputs=reuse_inputs) # keep dimensionless units until the end
    ...
    def step(self, reuse_inputs=True):
        ...
        for i, sim in enumerate(sims_to_update):
            ...
            if t_insts[i] > self.tmax[idx]:   # won't merge before max orbits, so update to that time
                self.sims[idx].t = self.tmax[idx]
            else:
        ...
        # convert units back to original units for final sims
        #self.sims = revert_sim_units(self.sims)
    ...
    def get_unstable_trios(self, sims):
        ...
        for i in range(len(sims)):
            for j in range(Npls[i] - 2):
                ...
                trio_sims.append(sim_subset(sims[i], [j+1, j+2, j+3])) # TODO: no longer scaling here. Should be OK bc deepreg scale invariant
    ...
    def handle_mergers(self, sims, trio_inds, updated_times, reuse_inputs=False):
    ...
    def _get_orbsmax(self, tmax):
        if tmax:    # used passed value
            ...
            tmax = [1e9*sim.particles[1].P for sim in self.sims]

        for i, t in enumerate(tmax):
            orbsmax = t/self.sims[i].particles[1].P
            if orbsmax > 1.01e9:
